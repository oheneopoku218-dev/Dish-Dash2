 const API_BASE = "https://ideal-pancake-x5g9g655wwgphrpr-5000.app.github.dev";
  ...
  form.addEventListener("submit", async (e) => {
    ...
    const steps = [...document.querySelectorAll(".step-input")].map(i =>
  i.value.trim());

    const recipeData = {
      title: document.getElementById("title").value.trim(),
      ingredients: document.getElementById("ingredients").value.split(",").map(i =>
  i.trim()),
      steps,
      cookingTime: Number(document.getElementById("cookingTime").value),
      difficulty: document.getElementById("difficulty").value,
      mealType: document.getElementById("mealType").value,
      dietaryTags: document.getElementById("dietaryTags").value.split(",").map(t =>
